...
class DetBenchTrainImagePair(DetBenchTrain):
    ...
    def forward(self, thermal_img, rgb_img, target: Dict[str, torch.Tensor], rgb_weight, thermal_weight, eval_pass=False, branch='fusion'):
        ...
        class_out, box_out, out, out_pred = self.model([thermal_img, rgb_img], rgb_weight, thermal_weight, branch=branch)

        loss_quality = []
        for i in range(len(out)):
            loss_quality.append(1 - F.cosine_similarity(out_pred[i].flatten(1), out[i].flatten(1)).mean())
        loss_quality_avg = sum(loss_quality) / len(loss_quality)
        ...
        if eval_pass:
            ...
            loss, class_loss, box_loss = self.loss_fn(class_out, box_out, cls_targets, box_targets, num_positives)
            #output['detections'] = _batch_detection(
            output = {'loss': loss+loss_quality_avg, 'class_loss': class_loss, 'box_loss': box_loss, 'detections': _batch_detection(
                thermal_img.shape[0], class_out_pp, box_out_pp, self.anchors.boxes, indices, classes,
                target['img_scale'], target['img_size'],
                max_det_per_image=self.max_det_per_image, soft_nms=self.soft_nms)}
        else:
            loss, class_loss, box_loss = self.loss_fn(class_out, box_out, cls_targets, box_targets, num_positives)
            output = {'loss': loss+loss_quality_avg, 'class_loss': class_loss, 'box_loss': box_loss}
        return output
